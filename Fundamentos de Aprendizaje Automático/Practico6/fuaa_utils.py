...
#########################################################################
def mostrar_sennales(path_audios, n_fold, n_clase):
    '''
...
def plot_features(features, labels, op_type1_1, op_type2_1, objects):
    ...
    plt.figure(figsize=_FIGSIZE, dpi=100)
...
def generar_conjunto_entrenamiento(folds, clases, op_feat1, op_feat2, folders_features, folders, objects):
    '''
    La función generar_conjunto_entrenamiento genera un conjunto de entrenamiento.

    Entrada:
        folds: lista con los folds a utilizar (int)
        clases: lista con las clases a utilizar (int)
        ...
        folders_features: características calculadas
        folders: información de los folds
        objects: clases a utilizar

    Salida:
        features: características calculadas
        labels: etiquetas asociadas a las características
    '''

    features = []  # Lista para almacenar las características
    labels = []    # Lista para almacenar las etiquetas asociadas a las características
    
    for c in clases:
        for fold in folds:
            # Filtra las características por clase y fold
            f_c = folders_features[fold-1][folders[fold-1][:, -2] == str(c)]
            
            # Realiza operaciones en las características, como ZCR y RMS
            f_c = features_from_arrays(f_c, tipo_c1=op_feat1, tipo_c2=op_feat2)
            
            # Agrega las características a la lista de características
            features.append(f_c)
            
            # Agrega las etiquetas correspondientes a la lista de etiquetas
            labels.append(c * np.ones(len(f_c)))
    
    # Concatena todas las características y etiquetas para formar matrices unificadas
    features = np.concatenate(features, axis=0)
    labels = np.concatenate(labels, axis=0)
    
    # Llama a una función para trazar o visualizar las características con etiquetas
    plot_features(features, labels, op_feat1, op_feat2, objects)
    
    # Retorna las características y etiquetas unificadas
    return features, labels

#########################################################################
def mostrar_superficie_decision(features, labels, clf_svm, clf_logreg, objects, op_type1_1, op_type2_1):
    ''' 
...
def mostrar_grid_search_results(rangoC, rangoGamma, resultados):
    ...
    rangoC_labels = [str("{:.2e}".format(c)) for c in rangoC]
    ax.set_yticklabels(rangoC_labels)
    ax.set_xlabel('Gamma')
    rangoGamma_labels = [str("{:.2e}".format(gamma)) for gamma in rangoGamma]
